...
import type { Gift, GiftStatistics, GiftFilters, GiftForm, Pagination, ApiResponse } from '~/types'
...
export const useGiftStore = defineStore('gift', {
  ...
  getters: {
    outgoingGifts: (state) => state.gifts.filter(g => g.gift_type === 'outgoing'),

    incomingGifts: (state) => state.gifts.filter(g => g.gift_type === 'incoming'),
    ...
    netOutgoing: (state) => state.totalOutgoingAmount - state.totalIncomingAmount,

    getGiftById: (state) => (id: number) => state.gifts.find(g => g.id === id),
  },

  actions: {
    async fetchGifts(filters?: GiftFilters, page: number = 1, pageSize: number = 20) {
      this.loading = true
      try {
        const api = useApi()
        ...
        if (res.success) {
          ...
          this.gifts = list.map(gift => ({
            ...gift,
            amount: gift.amount !== null ? parseFloat(String(gift.amount)) : null,
            item_value: gift.item_value !== null ? parseFloat(String(gift.item_value)) : null,
          }))
          this.pagination = {
    ...
    async createGift(data: GiftForm) {
      ...
      try {
        ...
        if (res.success) {
          await this.fetchGifts(this.filters || {}, this.pagination.page, this.pagination.pageSize)
          return res.data
        } else {
    ...
    async markAsReturned(id: number, isReturned: boolean, returnGiftId?: number) {
      this.loading = true
      try {
        const api = useApi()
    ...
    async fetchStatistics(startDate: string, endDate: string) {
      ...
      try {
        const api = useApi()
        const res: ApiResponse<GiftStatistics> = await api.get('/api/gift/statistics', {
